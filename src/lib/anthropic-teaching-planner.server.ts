...
export async function generateWithClaude(system: string, prompt: string): Promise<{ plan: TeachingPlan; usage: unknown; model: string }> {
  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", { method:"POST", signal:controller.signal, headers:{"content-type":"application/json","x-api-key":key,"anthropic-version":"2023-06-01"}, body:JSON.stringify({ model:process.env.ANTHROPIC_MODEL || "claude-sonnet-4-5", max_tokens:6000, temperature:0.2, system, messages:[{role:"user",content:prompt}] }) });
